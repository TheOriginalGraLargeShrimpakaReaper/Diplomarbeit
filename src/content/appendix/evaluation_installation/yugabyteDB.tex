...
\label{subsec:evaluation_installation_yugabytedb}
\subsubsection{Prerequisites}
%\paragraph{Prometheus Daten}
...
Das vollständige \gls{helm} Chart Manifest:
\lstset{style=gra_codestyle}
...
\lstset{style=gra_codestyle}
\begin{lstlisting}[language=bash, caption=yugabyteDB - Deinstallieren,captionpos=b,label={lst:yugabytedb-deinstall-cleanup},breaklines=true]
helm delete yw-evaluation -n yb-platform
kubectl delete pvc --namespace yb-platform -l app=yb-master
kubectl delete pvc --namespace yb-platform -l app=yb-tserver
kubectl delete namespace yb-platform
kubectl delete pv yb-storage-pv
kubectl delete storageclass yb-storage
\end{lstlisting}

\paragraph{StorageClass setzen}
Zuerst muss die StorageClass und das PersistentVolume gesetzt werden.
Wichtig ist hier, dass die Node Aiffinity gesetzt wird, damit die Persistence Volumes auf den Node geschrieben werden:
\lstset{style=gra_codestyle}
\begin{lstlisting}[language=yaml, caption=yugabyteDB - StorageClass setzen,captionpos=b,label={lst:yugabytedb-storageclass_250gib.yaml},breaklines=true]
...
\lstset{style=gra_codestyle}
\begin{lstlisting}[language=bash, caption=yugabyteDB - StorageClass / PersistentVolume Grosse Volumes aktivieren,captionpos=b,label={lst:yugabytedb-storageclass-big-apply},breaklines=true]
kubectl apply -f /home/gramic/PycharmProjects/rke2_settings/yugabytedb/yugabytedb/storageclass_250gib.yaml
\end{lstlisting}

\paragraph{Installation - 250GiB}
Zuerst muss wieder der Namespace erstellt werden:
\lstset{style=gra_codestyle}
\begin{lstlisting}[language=bash, caption=yugabyteDB - Namespace 250GiB,captionpos=b,label={lst:yugabytedb-namespace-250gib},breaklines=true]
kubectl create namespace yb-platform
\end{lstlisting}

Das \texttt{values.yaml} wurde für den letzten grossen Test angepasst.\\
Es werden nun 12GiB Memory allokiert für die \texttt{tserver} Nodes.\\
Es werden PVCs erstellt mit 240GiB Volume:
\lstset{style=gra_codestyle}
\begin{lstlisting}[language=yaml, caption=yugabyteDB - Helm Chart Manifest 250GiB,captionpos=b,label={lst:yugabytedb-250gib-values.yaml},breaklines=true]
# Default values for yugabyte.
# This is a YAML-formatted file.
# Declare variables to be passed into your templates.
Component: "yugabytedb"
...
gflags:
  master:
    default_memory_limit_to_ram_ratio: 0.85
  tserver:
    ysql_beta_features: true                      # gramic, wird für vacuum benötigt
  follower_unavailable_considered_failed_sec: 300 # gramic, autobalancing auf 5 minuten heruntersetzen
...
\label{subsubsec:yugabytedb_benchmarking_sql}
...
Die Tablespaces werden auf die drei Tablet-Server verteilt.\\
...
\lstset{style=gra_codestyle}
\begin{lstlisting}[language=sql, caption=yugabyteDB - Cloud - Region - Zone,captionpos=b,label={lst:yugabytedb-get-cloud-region-zone},breaklines=true]
select
    cloud,
    region,
    zone
...
--  Tabelle pgbench_eval_bench erstellen
...
CREATE TABLESPACE eval_index_tablespace WITH (replica_placement='{"num_replicas": 3, "placement_blocks": [ {"cloud":"cloud1","region":"datacenter1","zone":"rack1","min_num_replicas":3}]}');

--  Genereller Tablespace erstellen
drop tablespace if exists eval_data_tablespace;
CREATE TABLESPACE eval_data_tablespace WITH (replica_placement='{"num_replicas": 3, "placement_blocks": [ {"cloud":"cloud1","region":"datacenter1","zone":"rack1","min_num_replicas":3}]}');
\end{lstlisting}
Die Tabellen werden automatisch von \texttt{ysql\_bench} beim Initialisieren erstellt,\\
daher sind keine weiteren Schritte notwendig.

Die Grösse der Tabellen lässt sich mit folgendem SQL auslesen:
\lstset{style=gra_codestyle}
\begin{lstlisting}[language=sql, caption=yugabyteDB - Benchmarking - Table Size,captionpos=b,label={lst:yugabytedb-benchmarking-table-sizes},breaklines=true]
select
  table_name,
...
Auch hier soll ein Index-Tablespace (\texttt{self\_healing\_indices\_tablespace}) und ein Daten-Tablespace (\texttt{self\_healing\_datas\_tablespace}) erstellt werden.\\
Neu dazu kommt der Longtext-Tablespace \texttt{self\_healing\_longtexts\_tablespace}.
...
Die Tabellen müssen entsprechend den Schemas erstellt werden, wobei einige Tabellen in 3 Tablets pro \texttt{tserver} gesplittet werden sollen (Also insgesamt 9).
Das gesamte \texttt{CREATE}-Skript:
\lstset{style=gra_codestyle}
\begin{lstlisting}[language=sql, caption=YugabyteDB - Self Healing Tests - CREATE-SQL,captionpos=b,label={lst:yugabytedb-self-healing-create-sql},breaklines=true]
--  self-healing-Tabelle
create database self_healing_test;

--  Tablespace für Indices
drop tablespace if exists self_healing_indices_tablespace;
CREATE TABLESPACE self_healing_indices_tablespace WITH (replica_placement='{"num_replicas": 3, "placement_blocks": [ {"cloud":"cloud1","region":"datacenter1","zone":"rack1","min_num_replicas":3}]}');

--  Genereller Tablespace erstellen
drop tablespace if exists self_healing_datas_tablespace;
CREATE TABLESPACE self_healing_datas_tablespace WITH (replica_placement='{"num_replicas": 3, "placement_blocks": [ {"cloud":"cloud1","region":"datacenter1","zone":"rack1","min_num_replicas":3}]}');

--  longtext Tablespace erstellen
drop tablespace if exists self_healing_longtexts_tablespace;
CREATE TABLESPACE self_healing_longtexts_tablespace WITH (replica_placement='{"num_replicas": 3, "placement_blocks": [ {"cloud":"cloud1","region":"datacenter1","zone":"rack1","min_num_replicas":3}]}');
...
grant all on all tables in schema hrm to yadmin;
grant all on all tables in schema accountands to yadmin;
grant all on all tables in schema customer_service_officers to yadmin;
grant all on all tables in schema generell to yadmin;
...
\lstset{style=gra_codestyle}
\begin{lstlisting}[language=sql, caption=YugabyteDB - Self Healing Tests - Init Data,captionpos=b,label={lst:yugabytedb-self-healing-init-data},breaklines=true]
...
\lstset{style=gra_codestyle}
\begin{lstlisting}[language=sql, caption=YugabyteDB - Self Healing Tests - Failover Data,captionpos=b,label={lst:yugabytedb-self-healing-failover-data},breaklines=true]
...
\lstset{style=gra_codestyle}
\begin{lstlisting}[language=sql, caption=YugabyteDB - Self Healing Tests - Recovery Data,captionpos=b,label={lst:yugabytedb-self-healing-recovery-data},breaklines=true]
select * from customer_service_officers.self_healing_accounts;
select * from hrm.self_healing_employees;
select * from accountands.self_healing_accountand_protocol;
select * from generell.self_healing_intranet;
