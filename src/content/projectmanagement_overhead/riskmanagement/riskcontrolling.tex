%! Author = gramic
%! Date = 21.03.24

% Preamble
%    \KOMAoptions{paper=A3,paper=landscape,DIV=20}
%\storeareas\landscapevaluesbig
%    \begin{landscape}
    \clearpage
    \KOMAoptions{paper=A3,paper=landscape,pagesize,DIV=20}
    \recalctypearea
\begin{flushleft}
    \subsection{Riskcontrolling}
    \subsubsection{Neu erfasste Risiken}
    \input{content/latex_tables/riskcontrolling-new-risks}
%    \end{landscape}
%    \begin{landscape}
        \subsubsection{Assessment 21.03.2024}
        \input{content/latex_tables/risk-assessments-21-03-2024}
%    \end{landscape}
\end{flushleft}
\clearpage
\KOMAoptions{paper=A4,paper=portrait,pagesize,DIV=20}
\recalctypearea
\begin{flushleft}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.75\linewidth]{source/riskmatrix/project-assessment-21-03-2024}
        \caption{Riskikomatrix - Assessment 21.03.2024}
        \label{fig:project-assessment-21-03-2024}
    \end{figure}
\end{flushleft}
\clearpage
\KOMAoptions{paper=A3,paper=landscape,pagesize,DIV=20}
\recalctypearea
\begin{flushleft}
    \subsubsection{Assessment 29.04.2024}
    \input{content/latex_tables/risk-assessments-24-09-2024}
\end{flushleft}
\clearpage
\KOMAoptions{paper=A4,paper=portrait,pagesize}
\recalctypearea
\begin{flushleft}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.75\linewidth]{source/riskmatrix/project-assessment-29-04-2024}
        \caption{Riskikomatrix - Assessment 29.04.2024}
        \label{fig:project-assessment-29-04-2024}
    \end{figure}
\end{flushleft}
\clearpage
\KOMAoptions{paper=A3,paper=landscape,pagesize,DIV=20}
\recalctypearea
\begin{flushleft}
    \subsubsection{Assessment 13.05.2024 / Abschluss}
    \input{content/latex_tables/risk-assessments-13-05-2024}
\end{flushleft}
\clearpage
\KOMAoptions{paper=A4,paper=portrait,pagesize}
\recalctypearea
\begin{flushleft}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.75\linewidth]{source/riskmatrix/project-assessment-13-05-2024}
        \caption{Riskikomatrix - Assessment 13.05.2024}
        \label{fig:project-assessment-13-05-2024}
    \end{figure}
\end{flushleft}